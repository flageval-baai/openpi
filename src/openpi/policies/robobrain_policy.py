...
class RoboInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        base_image = _parse_image(data["observation.image_front"])
        ...
        if "actions" in data:
            ...
            inputs["actions"] = raw_actions[:, [8,9,10,11,12,13,7]]
            #print(inputs["actions"])
            #print(inputs["actions"].shape)
        #print(data.keys())
        if "prompt" in data:
            ...
            inputs['prompt'] = 'Place the object that is closest to the apple into the basket'
        return inputs


@dataclasses.dataclass(frozen=True)
class RoboOutputs(transforms.DataTransformFn):
    def __call__(self, data: dict) -> dict:
